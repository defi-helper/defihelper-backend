...
export default async (process: Process) => {
  ...
  if (Object.keys(order.balances).length === 0) {
    if (process.task.attempt > 30) {
      throw new Error('Order balances not filled');
    }
  ...
  await container.telegram().send(
    'smartTradeOrderCreated',
    {
      name: inToken && outToken ? `${inToken.name}/${outToken.name}` : '',
      balance: inToken ? `for ${order.balances[inToken.id]} ${inToken.name}` : '',
    },
    Number(chatId),
    user.locale,
  );
